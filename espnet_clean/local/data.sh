...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    utils/subset_data_dir.sh --last data/train 46 data/deveval
    utils/subset_data_dir.sh --last data/deveval 2 data/${eval_set}
    utils/subset_data_dir.sh --first data/deveval 44 data/${train_dev}
    n=$(( $(wc -l < data/train/wav.scp) - 46 ))
