...
if ! "${skip_eval}"; then
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        ...
        for dset in ${test_folder}; do
            ...
            # 0. Copy feats_type
            cp "${model_path}/feats_type" "${_dir}/feats_type"

            # 1. Split the key file
            key_file=${test_folder}/text
            split_scps=""
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/tts_inference.JOB.log \
                ${python} -m espnet2.bin.tts_inference \
                    --ngpu "${_ngpu}" \
                    --data_path_and_name_and_type "${test_folder}/text,text,text" \
                    --data_path_and_name_and_type ${_speech_data}/${_scp},speech,${_type} \
                    --key_file "${_logdir}"/keys.JOB.scp \
                    --model_file "${model_path}"/"${inference_model}" \
                    --train_config "${model_path}"/config.yaml \
